...
DATABASE_URL = "sqlite:///qa_database.db" # Nome do arquivo do banco de dados SQLite
Base = declarative_base() # Base para as classes de modelo
...
class QAEntry(Base):
    ...
    def __repr__(self):
        return f"<QAEntry(question='{self.question[:30]}...', answer='{self.answer[:30]}...')>"
...
# --- Novas Funções para interação com o Banco de Dados SQL ---

def get_answer_from_db(question):
    """
    Recupera uma resposta para uma pergunta do banco de dados SQL.
    Esta função será uma ferramenta para o LLM.
    """
    session = Session()
    # Busca por perguntas que contenham a string da pergunta do usuário (case-insensitive)
    qa_entry = session.query(QAEntry).filter(QAEntry.question.ilike(f"%{question}%")).first()
    session.close()
    if qa_entry:
        push(f"Found answer in DB for: {question}")
        return {"found_answer": qa_entry.answer, "source": "SQL_DB"}
    push(f"No answer found in DB for: {question}")
    return {"found_answer": None}

def add_qa_to_db(question, answer):
    """
    Adiciona uma nova pergunta e resposta ao banco de dados SQL.
    Esta função será uma ferramenta para o LLM.
    """
    session = Session()
    try:
        # Verifica se a pergunta já existe para evitar duplicatas
        existing_qa = session.query(QAEntry).filter_by(question=question).first()
        if existing_qa:
            existing_qa.answer = answer # Atualiza a resposta se a pergunta já existe
            session.add(existing_qa)
            push(f"Updated QA in DB: {question}")
            return {"status": "updated", "question": question, "answer": answer}
        else:
            new_qa = QAEntry(question=question, answer=answer)
            session.add(new_qa)
            push(f"Added new QA to DB: {question}")
            return {"status": "added", "question": question, "answer": answer}
        session.commit()
    except Exception as e:
        session.rollback()
        push(f"Error adding/updating QA in DB: {e}")
...
get_answer_from_db_json = {
    "name": "get_answer_from_db",
    "description": "Use this tool to retrieve a predefined answer from a knowledge base of common questions and answers. Always prioritize this tool for common questions.",
    "parameters": {
        "type": "object",
        "properties": {
            "question": {
                "type": "string",
                "description": "The question to look up in the database. Try to match the exact wording of a common question as much as possible."
            }
        },
        "required": ["question"],
        "additionalProperties": False
...
add_qa_to_db_json = {
    "name": "add_qa_to_db",
    "description": "Use this tool to add a new common question and its answer to the knowledge base. Only use this when you are sure you have a definitive and concise answer to a user's question, and it is a question that might be asked frequently.",
    "parameters": {
        "type": "object",
        "properties": {
            "question": {
                "type": "string",
                "description": "The common question to be added."
            },
            "answer": {
                "type": "string",
                "description": "The definitive answer to the common question."
            }
        },
...
tools = [{"type": "function", "function": record_user_details_json},
        ...
        {"type": "function", "function": get_answer_from_db_json}, # Nova ferramenta
        {"type": "function", "function": add_qa_to_db_json}] # Nova ferramenta

# Adicione esta função em algum lugar do seu arquivo, talvez fora da classe Me para simplicidade,
# ou como um método estático/de classe dentro dela se preferir.
def list_all_qa_entries():
    """
    Retorna uma lista de todas as perguntas e respostas armazenadas no banco de dados.
    """
    session = Session()
    qa_entries = session.query(QAEntry).all()
    session.close()
    
    qa_list = []
    for entry in qa_entries:
        qa_list.append({"question": entry.question, "answer": entry.answer})
    return qa_list
...
if __name__ == "__main__":
    ...
    # Para testar, você pode chamar a função após o launch do Gradio ou em um script separado
    print("\n--- Perguntas e Respostas Frequentes Cadastradas ---")
    frequent_questions = list_all_qa_entries()
    if frequent_questions:
        for i, qa in enumerate(frequent_questions):
            print(f"{i+1}. Pergunta: {qa['question']}")
            print(f"   Resposta: {qa['answer']}")
            print("-" * 30)
    else:
        print("Nenhuma pergunta frequente cadastrada ainda.")